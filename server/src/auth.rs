...
pub(crate) const MAX_CODE_LEN: usize = 12;
...
pub async fn code_verify(
    ...
    let email = body.email.trim().to_lowercase();
    if email.len() > MAX_EMAIL_LEN || body.code.trim().len() > MAX_CODE_LEN {
...
pub async fn login(
    ...
    let email = body.email.trim().to_lowercase();
    // Validate input length before DB query or Argon2 hashing to prevent CPU/DB exhaustion DoS.
