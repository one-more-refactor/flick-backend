...
async fn identity_fields_are_length_bounded() {
    ...
    // lookup returns exists: false for oversized email
    let resp = send(
        &app,
        json_request(
            "POST",
    ...
    // code/request accepts oversized email with silent 204
    let resp = send(
        &app,
        json_request(
            "POST",
    ...
    // code/verify rejects oversized email or code with 400
    let resp = send(
        &app,
        json_request(
            "POST",
    ...
    let resp = send(
        ...
        json_request(
            ...
            "/api/auth/code/verify",
            None,
            json!({"email": "user@example.com", "code": "x".repeat(100)}),
        ),
    )
    .await;
    assert_eq!(resp.status(), StatusCode::BAD_REQUEST);

    // login rejects oversized email or password with 401
    let resp = send(
        &app,
        json_request(
            "POST",
    ...
    let resp = send(
        ...
        json_request(
            ...
            "/api/auth/login",
            None,
            json!({"email": "user@example.com", "password": "x".repeat(5000)}),
        ),
    )
    .await;
    assert_eq!(resp.status(), StatusCode::UNAUTHORIZED);

    // admin/login rejects oversized email or password with 401
    let resp = send(
        &app,
        json_request(
            "POST",
