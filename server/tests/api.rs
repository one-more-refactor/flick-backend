...
async fn identity_fields_are_length_bounded() {
    ...
    // Admin login with oversized inputs
    let resp = send(
        &app,
        json_request(
            "POST",
    ...
    // Code request with oversized email
    let resp = send(
        &app,
        json_request(
            "POST",
    ...
    // Code verify with oversized email or code
    let resp = send(
        &app,
        json_request(
            "POST",
    ...
    let resp = send(
        ...
        json_request(
            ...
            "/api/auth/code/verify",
            None,
            json!({"email": "user@example.com", "code": "c".repeat(50)}),
        ),
    )
    .await;
    assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
