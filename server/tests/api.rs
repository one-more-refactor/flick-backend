...
async fn identity_fields_are_length_bounded() {
    ...
    // login endpoints (user & admin) reject oversized fields immediately
    let resp = send(
        &app,
        json_request(
            "POST",
    ...
    // lookup, code request, and code verify length bounds
    let resp = send(
        &app,
        json_request(
            "POST",
    ...
    let resp = send(
        ...
        json_request(
            ...
            "/api/auth/code/verify",
            None,
            json!({"email": "valid@example.com", "code": "x".repeat(50)}),
        ),
    )
    .await;
    assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
