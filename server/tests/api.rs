...
async fn identity_fields_are_length_bounded() {
    ...
    // login, admin login, lookup, code request/verify reject oversized inputs immediately
    let huge_password = "x".repeat(5000);
    let resp = send(
        &app,
        json_request(
            "POST",
    ...
    let resp = send(
        ...
        json_request(
            ...
            "/api/admin/login",
            None,
            json!({"email": "admin@example.com", "password": huge_password}),
        ),
    )
    .await;
    assert_eq!(resp.status(), StatusCode::UNAUTHORIZED);
    ...
    let resp = send(
        ...
        json_request(
            ...
            "/api/auth/code/verify",
            None,
            json!({"email": "valid@example.com", "code": "1234567890123"}),
        ),
    )
    .await;
    assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
